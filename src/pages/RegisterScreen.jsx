...
export default function RegisterScreen() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^\d{4,6}$/.test(form.pin))
      return setError('PIN must be 4–6 digits (numbers only)');
    ...
    // BUG FIX #1 — sameEmail flag se clear message
    if (result.alreadyExists && result.sameEmail) {
      // Same email → sirf login karo
  ...
  // ── Kya error "login karo" wala hai ──
  const showLoginPrompt = error.toLowerCase().includes('already registered') ||
                          error.toLowerCase().includes('login instead');
    ...
    <div className="page-enter min-h-screen bg-white dark:bg-slate-900 flex flex-col safe-top safe-bottom">
      ...
      <form onSubmit={handleSubmit}
        ...
        {error && (
          ...
          className="stagger-1 dark:bg-red-900/20 dark:border-red-800 dark:text-red-400">
            {error}

            {/* Already registered → Login button dikhao */}
            {showLoginPrompt && (
              <button
                type="button"
